...
describe('Ledger API - ContractOperationVersionedVerifierKey', () => {
   ...
   *
   * @given A version string 'v4' and an operation verifier key
  ...
  test('should construct with version and verifier key', () => {
    ...
    expect(contractOperationVersionedVerifierKey.version).toEqual('v4');
    expect(contractOperationVersionedVerifierKey.toString(true)).toMatch(/V4\(VerifierKey\(.*/);
  });
});
