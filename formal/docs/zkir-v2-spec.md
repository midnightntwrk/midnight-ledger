...
**Versions covered:** ZKIR major version 2, minor versions **V0** (original release; bit-bound constraints enforced by explicit little-endian bit decomposition in-circuit) and **V1** (the default as of `midnight-zkir` 2.1; functionally identical to V0 at the preprocess level but with optimised Halo2 lowerings for `CondSelect`, `ConstrainBits`, and `ReconstituteField`, and more `pow2range` columns). Later minor versions — **V2** (Ledger 9, `midnight-zkir` ≥ 2.2, the default there), which is lowered through the zk-stdlib v2 proving pipeline — are out of scope for this document. The preprocess semantics (§4) does not depend on the minor version; only the in-circuit lowering does.
