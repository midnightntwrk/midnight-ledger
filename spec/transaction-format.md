...
`structure.rs`, tag `standard-transaction[v11]`:
...
one segment. `structure.rs`, tag `intent[v8]`:
...
`structure.rs`, tag `contract-action[v8]`:
...
* **`MaintenanceUpdate`** (tag `contract-maintenance-update[v2]`): authorised
...
enum TransactionResult {
    SucceedEntirely,
    FailEntirely,                                   // guaranteed section failed
    SucceedPartially { segment_success: Map<u16, bool> }, // must include segment 0 = true
}
```
...
* `GLOBAL_TAG = "midnight:"` is the fixed prefix; `<tag>` is the type's
  `Tagged::tag()`, e.g. `transaction[v11]`. Deserialization checks both, so
