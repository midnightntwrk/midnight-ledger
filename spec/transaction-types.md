...
* **Outcome:** `SucceedEntirely` / `FailEntirely` /
  `SucceedPartially{segment_success}` (guaranteed section is all-or-nothing;
  fallible segments roll back independently).
...
* **Tag:** `system-transaction[v8]` (`#[non_exhaustive]`).
...
(`ledger-parameters[v7]`), which embeds:

### `TransactionCostModel` (`transaction-cost-model[v4]`)
...
| `parallelism_factor` | divisor reflecting cross-transaction parallelism (initial: `4`) |
...
| `max_contract_metadata_size` | — | hard limit on the size of associated contract metadata |
...
|---|---|---|
| `Transaction` | `transaction[v11]` | §1 |
| `StandardTransaction` | `standard-transaction[v11]` | §2.1 |
| `ClaimRewardsTransaction` | `claim-rewards-transaction[v2]` | §2.2 |
| `SystemTransaction` | `system-transaction[v8]` | §3 |
...
| `TransactionCostModel` | `transaction-cost-model[v4]` | §5 |
| `TransactionLimits` | `transaction-limits[v3]` | §5 |
| `LedgerParameters` | `ledger-parameters[v7]` | §5 |
