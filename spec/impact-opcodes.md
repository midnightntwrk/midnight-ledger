...
| Operates on | ledger `StateValue`s (a stack) | typed values in named SSA memory (v2: a native-field-element tape) |
...
* **The public transcript.** The values a contract call returns via
  ...
  is what the ZK proof is bound to: in ZKIR, `PublicInput` consumes the
  public-transcript stream and `Impact` declares the public inputs the proof
  commits to. So `popeq` (Impact VM) and `PublicInput` / `Impact` (ZKIR) are
  the two ends of one channel. (In v2 this role was split across
  `PublicInput` + `DeclarePubInput` / `PiSkip`.)
* **Shared cryptographic primitives.** `CostModel` also prices proof / curve
  ...
  directly to ZKIR instructions (`EcMul`, `HashToCurve`, `TransientHash`, and
  point addition — `Add` on `JubjubPoint` in v3, `EcAdd` in v2).  These are
  charged when a transaction's proofs are verified, alongside the per-opcode
  Impact costs.
