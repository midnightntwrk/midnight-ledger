...
use super::ir::{Identifier, Instruction as I, IrSource};
...
impl IrSource {
    ...
    pub(crate) fn preprocess(
        ...
        let idx_bool = |memory: &HashMap<Identifier, Fr>, id: &Identifier| {
            idx(memory, id).and_then(|val| {
                if val == 0.into() {
                    Ok(false)
                } else if val == 1.into() {
                    Ok(true)
        ...
        let idx_point = |memory: &HashMap<Identifier, Fr>, x: &Identifier, y: &Identifier| {
            let x = idx(memory, x)?;
            let y = idx(memory, y)?;
            EmbeddedGroupAffine::new(x, y)
                .ok_or(anyhow!("Elliptic curve point not on curve: ({x:?}, {y:?})"))
        };
        let idx_bits = |memory: &HashMap<Identifier, Fr>, id: &Identifier, constrain: Option<u32>| {
            idx(memory, id).and_then(|val| {
                let mut bits = val
                    .0
                    .to_bytes_le()
        ...
        for ins in self.instructions.iter() {
            ...
            match ins {
                I::Add { a, b, output } => {
                    let result = idx(&memory, a)? + idx(&memory, b)?;
                ...
                I::Neg { a, output } => {
                    let result = -idx(&memory, a)?;
                    memory.insert(output.clone(), result);
                }
                I::Not { a, output } => {
                    let result = (!idx_bool(&memory, a)?).into();
                    memory.insert(output.clone(), result);
                }
                I::ConstrainEq { a, b } => {
                ...
                I::TestEq { a, b, output } => {
                    let result = (idx(&memory, a)? == idx(&memory, b)?).into();
                    memory.insert(output.clone(), result);
                }
                ...
                I::ReconstituteField {
                    ...
                    max_map.insert(Identifier::from_index(0), fr_max);
                    let max_bits = idx_bits(&max_map, &Identifier::from_index(0), None)?;
                    ...
                    let result = power * idx(&memory, divisor)? + idx(&memory, modulus)?;
                    memory.insert(output.clone(), result);
                }
                I::LessThan { a, b, bits, output } => {
                    let result = (from_bits(idx_bits(&memory, a, Some(*bits))?.into_iter())
        ...
        for ins in self.instructions.iter() {
            match ins {
                ...
                I::PublicInput { guard, output } | I::PrivateInput { guard, output } => {
                    ...
                    let value = witness.as_ref().map(|preproc| {
                        preproc.memory.get(output).copied().unwrap_or(0.into())
                    });
