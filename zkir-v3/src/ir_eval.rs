...
pub(crate) fn eval_computational_instruction(
    ...
    match ins {
        ...
        I::PersistentHash {
            ...
            // `HashOutput` is `[u8; 32]` and `FR_BYTES_STORED == 31`, so
            // `field_vec()` always yields exactly two `Fr`s.
            let [hash_hi, hash_lo]: [Fr; 2] = hash
                .field_vec()
                .try_into()
                .expect("persistent_hash output must split into exactly 2 Fr");
            memory.insert(outputs[0].clone(), IrValue::Native(hash_hi));
            memory.insert(outputs[1].clone(), IrValue::Native(hash_lo));
            Ok(Some(()))
        }
