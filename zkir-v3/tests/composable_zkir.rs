...
use base_crypto::fab::{AlignedValue, Alignment, AlignmentAtom, Value, ValueAtom};
...
use midnight_zkir_v3::ir_execute::{
    CallRole, ExecutionContext, ExecutionError, ExecutionResult, WitnessProvider, ZkirProvider,
    sub_calls_of,
};
...
fn addr_to_fr_vec(addr: ContractAddress) -> Vec<Fr> {
    let (hi, lo) = addr_to_frs(addr);
    vec![hi, lo]
}

/// Construct an `EntryPointBuf` from a string slice.
fn ep(name: &str) -> EntryPointBuf {
...
fn field_aligned_value(fr: Fr) -> AlignedValue {
    AlignedValue {
        value: Value(vec![ValueAtom(strip_trailing_zeros(
            fr.0.to_bytes_le().to_vec(),
        ))]),
...
#[actix_rt::test]
async fn test_execute_single_contract_ledger_read() {
...
async fn test_execute_cross_contract_call() {
    ...
    let calls: ExecutionResult<D> = outer_ir
        .execute(addr_to_fr_vec(inner_addr), context, &mut rng)
        .await
        .expect("execute should succeed");

    // Two-call tree: outer (root, idx 0) → inner (sub, idx 1)
    assert_eq!(calls.len(), 2, "expected two calls in the tree");
    let root = &calls[0];
    let sub = &calls[1];

    assert_eq!(root.address, outer_addr);
    assert_eq!(root.entry_point, ep("call_inner"));
    assert_eq!(sub.parent, Some(0), "inner's parent is outer");
    assert!(matches!(root.role, CallRole::Root));
    assert!(matches!(sub.role, CallRole::Sub { .. }));
    ...
    assert_eq!(sub.entry_point, ep("get"));
    assert_eq!(sub.output[0], stored_value);

    // Verify the callee produced a transcript
    assert!(!sub.program.is_empty(), "callee should have transcript ops");

    // Verify communication commitment randomness was generated (non-zero)
    assert_ne!(
        sub.comm_comm_rand().expect("sub has comm_comm_rand"),
        Fr::from(0u64),
        "comm rand should be non-zero"
    );

    // Caller's transcript should contain claim ops from ContractCall
    assert!(
        !root.program.is_empty(),
        "caller transcript should contain claim ops from ContractCall"
    );

    // Comm linkage: only the sub-call carries comm_comm
    assert!(root.comm_comm().is_none(), "root has no comm_comm");
...
#[actix_rt::test]
async fn test_witness_limitation_in_cross_contract_call() {
    ...
    let err = caller_ir
        .execute(addr_to_fr_vec(callee_addr), context, &mut rng)
        .await
        .map(|_| ())
...
#[actix_rt::test]
async fn test_private_input_at_top_level() {
    ...
    let calls: ExecutionResult<D> = ir
        .execute(vec![], context, &mut rng)
        .await
    ...
    let root = &calls[0];
    assert_eq!(root.output[0], witness_value);

    // Verify private transcript output was recorded — one AV per
    // PrivateInput instruction; this circuit has exactly one.
    assert_eq!(
        root.private_transcript_outputs().len(),
        1,
        "should record one private transcript AV"
    );
}

/// Test 5: Maximum call depth exceeded.
#[actix_rt::test]
async fn test_max_call_depth_exceeded() {
    ...
    let err = self_caller
        .execute(addr_to_fr_vec(addr), context, &mut rng)
        .await
        .map(|_| ())
...
#[actix_rt::test]
async fn test_execute_pure_computation() {
    ...
    assert_eq!(calls.len(), 1);
    let root = &calls[0];
    assert_eq!(root.output, vec![input_val]);
    assert!(root.program.is_empty());
}

/// Test 7: Input count mismatch is rejected.
#[actix_rt::test]
async fn test_input_count_mismatch() {
    ...
    match err {
        ExecutionError::Internal(msg) => {
            assert!(
                msg.contains("expected 1 input field elements, got 0"),
...
async fn test_provider_error_propagates() {
    ...
    let err = outer_ir
        .execute(addr_to_fr_vec(bogus_inner_addr), context, &mut rng)
        .await
        .map(|_| ())
...
/// in the execute path.
#[actix_rt::test]
async fn test_execute_arithmetic_instructions() {
    ...
    assert_eq!(calls.len(), 1);
    let root = &calls[0];
    let outputs = &root.output;
    assert_eq!(outputs.len(), 3);
    assert_eq!(outputs[0], Fr::from(12u64)); // 5 + 7
    assert_eq!(outputs[1], Fr::from(35u64)); // 5 * 7
    assert_eq!(outputs[2], -a); // -5
}

/// Test 10: Cross-contract call with outputs propagated back.
/// Two-level call: outer calls middle, middle calls inner.
#[actix_rt::test]
async fn test_execute_two_level_call_chain() {
    ...
    // Three calls in DFS preorder: outer (0) → middle (1) → inner (2)
    assert_eq!(calls.len(), 3, "three participants in the call tree");
    let outer_call = &calls[0];
    let middle_call = &calls[1];
    let inner_call = &calls[2];

    assert!(outer_call.parent.is_none());
    assert_eq!(middle_call.parent, Some(0));
    assert_eq!(inner_call.parent, Some(1));

    // Outer should return the value that propagated through middle → inner
    assert_eq!(outer_call.output, vec![inner_value]);
    assert_eq!(middle_call.address, middle_addr);
    assert_eq!(inner_call.address, inner_addr);
    assert_eq!(inner_call.output, vec![inner_value]);

    // Comm linkage: outer has none, middle and inner have comm_comm.
    assert!(outer_call.comm_comm().is_none());
    let middle_comm = middle_call.comm_comm().expect("middle has comm_comm");
    let inner_comm = inner_call.comm_comm().expect("inner has comm_comm");

    // Middle's comm_comm should appear in outer's claimed_contract_calls.
    let outer_claimed_comms: Vec<Fr> = outer_call
        .context
        .effects
    ...
    let middle_claimed_comms: Vec<Fr> = middle_call
        .context
        .effects
        .claimed_contract_calls
        .iter()
    ...
    // Sanity check: sub_calls_of helper agrees with parent indices.
    let outer_children: Vec<usize> = sub_calls_of(&calls, 0).map(|(i, _)| i).collect();
    let middle_children: Vec<usize> = sub_calls_of(&calls, 1).map(|(i, _)| i).collect();
    assert_eq!(outer_children, vec![1]);
    assert_eq!(middle_children, vec![2]);
}
...
#[actix_rt::test]
async fn test_claim_ops_produce_correct_effects() {
    ...
    let expected_ep_hash = ep("get").ep_hash();
    assert_eq!(
        *claimed_ep_hash, expected_ep_hash,
        "claim should have correct entry point hash"
    );

    // 3. The claimed comm_comm should match the callee's comm_comm.
    assert_eq!(calls.len(), 2);
    let callee_comm = calls[1].comm_comm().expect("callee should have comm_comm");
    assert_eq!(
    ...
    // `sub.input` and `sub.output` are flat `Vec<Fr>`, so the IO representation
...
#[actix_rt::test]
async fn test_nontrivial_result_from_call_parameter() {
    ...
    assert_eq!(inner_call.parent, Some(0));
    assert_eq!(inner_call.address, inner_addr);
    assert_eq!(inner_call.entry_point, ep("double"));
    assert!(outer_call.comm_comm().is_none());
    assert!(inner_call.comm_comm().is_some());
}

/// Test 13: Contract reads callee address from its own ledger state, then calls it.
#[actix_rt::test]
async fn test_call_contract_from_ledger_state() {
    ...
    // Outer contract A: reads inner_addr from state[0], then calls inner.
    let outer_ir = IrSource {
        inputs: vec![
    ...
    assert_eq!(calls.len(), 2, "two calls expected");
    let outer_call = &calls[0];
    let inner_call = &calls[1];
    assert_eq!(
        inner_call.address, inner_addr,
        "sub-call should target inner"
    );
    assert_eq!(
        inner_call.output,
        vec![Fr::from(8u64)],
    ...
    assert_eq!(inner_call.entry_point, ep("increment"));
    assert!(outer_call.comm_comm().is_none(), "root has no comm_comm");
    assert!(inner_call.comm_comm().is_some(), "callee has a comm_comm");
}

/// Test: Guarded PublicInput before unguarded PublicInput.
#[actix_rt::test]
async fn test_guarded_public_input_before_unguarded() {
    use std::borrow::Cow;
...
///
/// A JubjubPoint is encoded as 2 field elements. The flat-Fr API takes the
/// concatenation of all typed-input encodings — for a `JubjubPoint`, that's
/// `[x, y]` directly, with no `AlignedValue` wrapper.
#[actix_rt::test]
async fn test_execute_jubjub_point_input() {
    use group::Group;
    ...
    assert_eq!(
        calls[0].output,
        vec![px],
        "output should be the x coordinate"
    );
}
