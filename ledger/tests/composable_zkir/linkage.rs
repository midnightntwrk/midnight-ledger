...
use midnight_ledger::construct::partition_transcripts;
...
#[tokio::test]
async fn test_entry_point_hash_linkage() {
    ...
    // Extract the claimed entry from caller's (root's) effects
    let claimed: Vec<_> = result[0]
        .context
        .effects
...
#[tokio::test]
async fn test_partition_rejects_missing_callee() {
    ...
    // Only provide the caller's transcript (root, index 0), omitting the
    // callee. This means the caller has a claim that doesn't match any
    // callee in the PreTranscript list.
    let pre_transcripts: Vec<PreTranscript<D>> =
        vec![call_to_pre_transcript(result.into_iter().next().unwrap())];
