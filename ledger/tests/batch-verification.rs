...
#[tokio::test]
async fn batch_verification() {
    batch_verification_inner(TestMode::Full).await
}

#[allow(unused_assignments, clippy::redundant_clone)]
async fn batch_verification_inner(mode: TestMode) {
    ...
    let deferred_unbalanced_strictness = unbalanced_strictness.defer_proofs();
    let deferred_balanced_strictness = balanced_strictness.defer_proofs();
    ...
    let tx = mode
        .replay_or("tx1", async || {
            ...
            tx.well_formed(&state.ledger, deferred_unbalanced_strictness, state.time)
                .unwrap();
    ...
    // Part 5: advance to the reveal phase
