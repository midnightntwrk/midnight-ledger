...
> StandardTransaction<S, P, B, D>
{
    ...
    pub fn collect_proof_evidence(
        ...
        // Zswap evidence goes through the reference, exactly as the contract and dust evidence
        // above already does — `collect_proof_evidence` on those routes through `op_check` and
        // `dust_spend_check`, so a reference that knows the proofs were already checked returns
        // nothing for them and the aggregate check has nothing to re-verify.
        //
        // `zswap_collect_proof_evidence` takes no state: the evidence is a pure function of the
        // transaction bytes, which is why `stateless_check` is the right hook and why skipping it
        // for a byte-identical, already-verified transaction is sound. Under an ordinary
        // `LedgerState` the closure simply runs, so batching is unaffected.
        //
        // Do not inline these calls back out of the hook. Zswap was the one evidence source not
        // routed through the reference, and that asymmetry silently cost `RevalidationReference`
        // its ability to skip zswap proofs — the dominant verification cost for payment traffic.
        ref_state.stateless_check(|| {
