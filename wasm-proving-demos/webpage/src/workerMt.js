...
import init, { initThreadPool, provingProvider } from '@midnight-ntwrk/zkir-v2-mt';
...
onmessage = (m) => {
  (async () => {
    ...
    const prover = provingProvider(kmProvider);
    postMessage({ msg: "start", id });
    const provePromise = tx.prove(prover, CostModel.dummyCostModel());
    const provenTx = await provePromise;
    console.log(provenTx.toString());
    postMessage({ msg: "done", id });
  })()
};
