...
//! One accumulator that does not pair rejects the whole batch. Its proof is
//! valid, so PLONK passes and only the deferred pairing can refuse it.
...
fn batch_verify_rejects_bad_accumulator() {
    ...
    // Control: each behaves as expected on its own.
    good_vk
