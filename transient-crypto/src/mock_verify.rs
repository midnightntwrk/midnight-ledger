...
///
/// The extra pairing cost accounts for the deferred pairing check that
/// [`VerifierKey::verify`] performs on each accumulator exposed by a
/// `verify_proof` instruction, on top of `midnight_zk_stdlib::verify`.
...
/// iterations. The accumulator is arbitrary (bases are the generator, scalars
/// are one) — pairing wall time is content-oblivious as long as the inputs are
/// non-identity, which they are here.
