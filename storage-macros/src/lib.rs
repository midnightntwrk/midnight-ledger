...
/// generated `Storable` impl will return the `ChildNode` of any such annotated fields
...
pub fn derive_storable(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let mut expanded = quote! {
        ...
        impl #impl_generics Storable<#db> for #name #ty_generics #where_clause {
            fn children(&self) -> std::vec::Vec<ChildNode<<#db as DB>::Hasher>> {
                #children_impl
            }
            ...
            fn from_binary_repr<R: std::io::Read>(reader: &mut R, child_hashes: &mut impl Iterator<Item = ChildNode<<#db as DB>::Hasher>>, loader: &impl Loader<#db>) -> Result<Self, std::io::Error> {
    ...
    is_base.then(|| {
        ...
        quote! {
            impl #impl_tokens Storable<_D> for #name #ty_generics #where_clause {
                fn children(&self) -> std::vec::Vec<ChildNode<<_D as DB>::Hasher>> {
                    vec![]
                }
                ...
                fn from_binary_repr<R: std::io::Read>(reader: &mut R, _child_hashes: &mut impl Iterator<Item = ChildNode<<_D as DB>::Hasher>>, loader: &impl Loader<_D>) -> Result<Self, std::io::Error> {
